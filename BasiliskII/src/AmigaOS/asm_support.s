...
dissb:  
        dc.w    0x4e7a,0x0808           /* movec pcr,d0 - hand assembled for 68060 */
        bset    #5,%d0
        dc.w    0x4e7b,0x0808           /* movec d0,pcr - hand assembled for 68060 */
        rte
...
doTrapXX:
        ...
        move.l  8(%sp),%d0              /* vector number 32-47 */

        move.l  %usp,%a0                /* Get user stack pointer */
        move.l  16(%sp),-(%a0)          /* Copy 4-word stack frame to user stack */
        move.l  12(%sp),-(%a0)
...
/* andi #xxxx,sr */
andisr:         
        move.w  4(%sp),%d0              /* Get CCR from stack */
...
/* move #xxxx,sr */
movetosrimm:    
        move.w  8(%sp),%d0              /* Get immediate value */
        bra.s   storesr4
...
/* movec cr,x */
movecfromcr:    
        move.w  8(%sp),%d0              /* Get next instruction word */
...
/* movec x,vbr */
movectovbr:     
        move.l  (%sp)+,%d0              /* Ignore moves to VBR */
        addq.l  #4,2(%sp)
        rte

/* movec dx,cacr */
movectocacr:    
        movem.l %d1/%a0-%a1/%a6,-(%sp)  /* Move to CACR, clear caches */
        move.l  _SysBase,%a6
        jsr     -636(%a6)               /* CacheClearU */
...
/* movec x,dfc */
movectoxfc:     
        move.l  (%sp)+,%d0              /* Ignore moves to SFC, DFC */
        addq.l  #4,2(%sp)
        rte
...
dotrace:
        ...
        move.l  12(%sp),-(%a0)          /* Copy 6-word stack frame to user stack */
        move.l  8(%sp),-(%a0)
        move.l  4(%sp),-(%a0)
        move.l  %a0,%usp                /* Update USP */
        or.w    #0x2000,(%a0)           /* set Supervisor bit in SR */
        ...
        lea     12(%sp),%sp             /* Remove exception frame from supervisor stack */
...
pv_unhandled:   
        move.l  (%sp),%d0               /* Unhandled instruction, jump to handler in main.cpp */
        move.l  %a6,(%sp)               /* Save a6 */
        move.l  %usp,%a6                /* Get user stack pointer */
...
pushsr:         
        ...
        move.l  (%sp)+,%a0              /* Restore a0 */
        move.l  (%sp)+,%d0              /* Restore d0 */
        addq.l  #2,2(%sp)               /* Skip instruction by modifying return PC */
        rte
